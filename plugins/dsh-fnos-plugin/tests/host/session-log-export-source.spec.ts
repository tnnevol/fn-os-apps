 ...
 *
 * 因此断言两件事：请求确实打到那条 route（而不是某个注入服务），且地址是本机
 * loopback —— 绑定 0.0.0.0 时不能真的往 0.0.0.0 发请求。
 */
...
describe('「导出到 NAS」的上游取数', () => {
  it('请求 DSH 自己的 /api/session.export 而不是注入服务', async () => {
    ...
    await fetchSessionLogZip(context('127.0.0.1', 3080) as never, 'session-1', new AbortController().signal)
  ...
  it('绑定 0.0.0.0 时回落到 loopback', async () => {
    const calls = stubFetch(new Response('zip', { status: 200 }))
    await fetchSessionLogZip(context('0.0.0.0', 3080) as never, 'session-1', new AbortController().signal)
  ...
  it('用实际监听端口，而不是猜一个固定值', async () => {
    ...
    await fetchSessionLogZip(context('127.0.0.1', 51999) as never, 'session-2', new AbortController().signal)
  ...
  it('把中止信号透传给上游，客户端断开时一并取消', async () => {
    ...
    await fetchSessionLogZip(context('127.0.0.1', 3080) as never, 'session-1', controller.signal)
  ...
  it('上游非 2xx 时原样返回，由调用方决定对外状态码', async () => {
    ...
    const response = await fetchSessionLogZip(context('127.0.0.1', 3080) as never, 'missing', new AbortController().signal)
