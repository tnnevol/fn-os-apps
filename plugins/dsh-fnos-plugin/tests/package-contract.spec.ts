...
describe('dsh-fnos package contract', () => {
  ...
  it('replaces the fnOS Session log utility with a computer/NAS dropdown', async () => {
    ...
    const theme = await readFile(new URL('../../../packages/dsh-semi-ui/src/theme.ts', import.meta.url), 'utf8')
    ...
    expect(theme).toContain(':not([${DSH_DARK_THEME_ATTRIBUTE}]) .semi-button.semi-button-solid:not(.semi-button-disabled)')
    expect(theme).toContain('body[${SEMI_DSH_THEME_ATTRIBUTE}][${DSH_DARK_THEME_ATTRIBUTE}] .semi-button.semi-button-solid:not(.semi-button-disabled)')
  ...
  it('keeps the original DSH workspace flow and augments it with a shortcut', async () => {
    ...
    const restart = await readFile(new URL('../src/client/FnosWebRestartAction.tsx', import.meta.url), 'utf8')
    const clientStyle = await readFile(new URL('../src/client/style.css', import.meta.url), 'utf8')
    ...
    expect(semiTheme).toContain("const SEMI_THEME_MODE_ATTRIBUTE = 'theme-mode'")
    expect(semiTheme).toContain('data-ds-dark-theme')
    expect(semiTheme).toContain("setAttribute(SEMI_THEME_MODE_ATTRIBUTE, document.body.hasAttribute(DSH_DARK_THEME_ATTRIBUTE) ? 'dark' : 'light')")
    expect(semiTheme).toContain('MutationObserver')
    ...
    expect(clientStyle).toContain('.dsh-fnos-authorized-path-picker .semi-tree-option-list')
    ...
    expect(picker).toContain("border: 'none'")
    ...
    expect(semiTheme).toContain('border: none !important')
    expect(semiTheme).toContain('background: var(--dsw-static-neutral-bluish-00) !important')
    expect(semiTheme).toContain('border-color: var(--dsw-static-neutral-bluish-1000) !important')
    expect(semiTheme).toContain('color: var(--dsw-static-neutral-bluish-1000) !important')
    expect(semiTheme).toContain('background: var(--dsw-static-neutral-bluish-1000) !important')
    expect(semiTheme).toContain('border-color: var(--dsw-static-neutral-bluish-00) !important')
    expect(semiTheme).toContain('color: var(--dsw-static-neutral-bluish-00) !important')
    expect(semiTheme).toContain(':not([${DSH_DARK_THEME_ATTRIBUTE}]) .semi-button.semi-button-primary.semi-button-solid:not(.semi-button-disabled)')
    expect(semiTheme).toContain('background-color: var(--dsw-static-neutral-bluish-1000) !important')
    expect(semiTheme).toContain('color: var(--dsw-static-neutral-bluish-00) !important')
    expect(semiTheme).toContain('body[${SEMI_DSH_THEME_ATTRIBUTE}][${DSH_DARK_THEME_ATTRIBUTE}] .semi-button.semi-button-primary.semi-button-solid:not(.semi-button-disabled)')
    ...
    expect(semiTheme).toContain('background: transparent !important')
    expect(semiTheme).toContain('var(--dsw-alias-interactive-bg-hover) !important')
