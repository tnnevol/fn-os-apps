...
describe('dsh-fnos package contract', () => {
  it('declares a bundle and an early web client with theme ordering', async () => {
    ...
    expect(manifest.version).toBe('0.1.1-rc.2.2')
  ...
  it('replaces the fnOS Session log utility with a computer/NAS dropdown', async () => {
    ...
    expect(theme).toContain(':not([${DSH_DARK_THEME_ATTRIBUTE}]) .semi-button-primary:not(.semi-button-disabled)')
    expect(theme).toContain('body[${SEMI_DSH_THEME_ATTRIBUTE}][${DSH_DARK_THEME_ATTRIBUTE}] .semi-button-primary:not(.semi-button-disabled)')
    expect(theme).toContain('background: var(--dsw-static-neutral-bluish-1000) !important')
    expect(theme).toContain('background: var(--dsw-static-neutral-bluish-00) !important')
  ...
  it('keeps the original DSH workspace flow and augments it with a shortcut', async () => {
    ...
    expect(semiTheme).toContain('background: var(--dsw-alias-label-primary) !important')
    expect(semiTheme).toContain('color: var(--dsw-alias-label-primary-foreground) !important')
