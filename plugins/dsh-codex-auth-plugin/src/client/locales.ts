/** Browser copy for the standalone Codex authentication card. */

export const en = {
  title: 'Codex Auth',
  intro: 'Sign in with your ChatGPT account for Codex-compatible plugins.',
  expand: 'Expand settings',
  collapse: 'Collapse settings',
  loading: 'Loading account…',
  signedOut: 'Not signed in',
  signingIn: 'Waiting for Codex authorization…',
  signedIn: 'Signed in',
  signIn: 'Sign in',
  signOut: 'Sign out',
  cancelSignIn: 'Cancel',
  working: 'Working…',
  popupBlocked: 'The browser blocked the sign-in window. Allow pop-ups for this dsh page and retry.',
  authorizationWindowClosed: 'The authorization window was closed before sign-in finished. Start again when you are ready.',
  requestFailed: 'The Codex account request failed.',
  remoteOrigin: 'This browser origin is not trusted by the DSH Web server.',
  authorizationCodeHelp: 'The plugin generated a one-time code. Click the copy icon next to it, then enter the code on the Codex authorization page; no workspace selection is required.',
  authorizationCodeLabel: 'Codex authorization code',
  copyAuthorizationCode: 'Copy',
  authorizationCodeCopied: 'Copied',
  authorizationCodeCopyFailed: 'Copying was blocked by the browser. Select the code and copy it manually.',
  openAuthorization: 'Open authorization page',
  usageLoading: 'Loading usage…',
  usageUnavailable: 'Usage information is temporarily unavailable.',
  usageFiveHour: 'Five-hour usage limit',
  usageWeekly: 'Weekly usage limit',
  usageStatusRemaining: 'Remaining',
  usageStatusReset: 'Reset',
  settingsLoading: 'Loading plugin settings…',
  settingsSaved: 'Saved',
  settingsSaveFailed: 'Unable to save settings.',
  cancel: 'Cancel',
  discard: 'Discard',
  save: 'Save',
  saving: 'Saving…',
  globalModelTitle: 'Global model',
  globalModelIntro: 'Choose the default Codex model for new DSH sessions.',
  globalModelModel: 'Model',
  globalModelThinking: 'Reasoning effort',
  globalModelProviderDefault: 'Provider default',
  globalModelUnset: 'Select a model',
  globalModelLoadFailed: 'Unable to load the Codex model catalog.',
  globalModelUnavailable: 'This model is no longer in the current catalog.',
  setGlobalModel: 'Set as global model',
  modelRefreshTitle: 'Model catalog',
  modelRefreshIntro: 'Fetch the models and reasoning efforts your ChatGPT account can use right now and sync them into DSH.',
  modelRefreshAction: 'Refresh model catalog',
  modelRefreshing: 'Refreshing…',
  modelRefreshDone: 'Synced {count} models from your ChatGPT account.',
  modelRefreshFailed: 'Unable to refresh the Codex model catalog. Check the sign-in state and try again.',
}

export type CodexAuthLocaleKey = keyof typeof en

export const zh: { [Key in CodexAuthLocaleKey]: string } = {
  title: 'Codex Auth',
  intro: '使用 ChatGPT 账户登录，为 Codex 兼容插件提供认证状态。',
  expand: '展开设置',
  collapse: '折叠设置',
  loading: '正在加载账户信息…',
  signedOut: '尚未登录',
  signingIn: '正在等待 Codex 授权…',
  signedIn: '已登录',
  signIn: '登录',
  signOut: '退出登录',
  cancelSignIn: '取消',
  working: '处理中…',
  popupBlocked: '浏览器阻止了登录窗口。请允许此 dsh 页面弹出窗口后重试。',
  authorizationWindowClosed: '授权窗口已关闭，登录未完成。准备好后可重新登录。',
  requestFailed: 'Codex 账户请求失败。',
  remoteOrigin: '当前浏览器来源未被 DSH Web 服务信任。',
  authorizationCodeHelp: '插件已生成一次性授权码。点右侧复制图标复制，再到 Codex 授权页面粘贴；无需选择工作空间。',
  authorizationCodeLabel: 'Codex 授权码',
  copyAuthorizationCode: '复制',
  authorizationCodeCopied: '已复制',
  authorizationCodeCopyFailed: '浏览器阻止了复制，请手动选中授权码复制。',
  openAuthorization: '打开授权页面',
  usageLoading: '正在加载用量…',
  usageUnavailable: '暂时无法获取 Codex 用量信息。',
  usageFiveHour: '五小时使用限额',
  usageWeekly: '每周使用限额',
  usageStatusRemaining: '剩余',
  usageStatusReset: '重置',
  settingsLoading: '正在加载插件设置…',
  settingsSaved: '已保存',
  settingsSaveFailed: '设置保存失败。',
  cancel: '取消',
  discard: '放弃',
  save: '保存',
  saving: '保存中…',
  globalModelTitle: '全局模型',
  globalModelIntro: '设置新 DSH 会话使用的 Codex 默认模型。',
  globalModelModel: '模型',
  globalModelThinking: '思考强度',
  globalModelProviderDefault: '提供方默认',
  globalModelUnset: '请选择模型',
  globalModelLoadFailed: '加载 Codex 模型目录失败。',
  globalModelUnavailable: '当前模型已不在可用目录中。',
  setGlobalModel: '设置为全局模型',
  modelRefreshTitle: '模型目录',
  modelRefreshIntro: '从你的 ChatGPT 账号获取当前可用的模型与思考强度，并同步到 DSH。',
  modelRefreshAction: '刷新模型目录',
  modelRefreshing: '刷新中…',
  modelRefreshDone: '已从账号同步 {count} 个模型。',
  modelRefreshFailed: '无法刷新 Codex 模型目录，请确认登录状态后重试。',
}
