...
describe('dsh-codex-auth-plugin alpha.4 client registration', () => {
  ...
  it('adds the Codex-only model picker enhancements without changing the official source', async () => {
    const presentation = await readFile(new URL('../../src/client/services/model-editor-presentation.ts', import.meta.url), 'utf8')
    ...
    expect(style).toContain('--dsw-alias-button-primary-fill')
    ...
    expect(style).toContain('gap: 15px')
    ...
    expect(style).toContain('background: var(--dsw-alias-bg-layer-1)')
    ...
    expect(style).toContain('accent-color: var(--dsw-alias-button-primary-fill)')
    expect(style).toContain('var(--dsw-alias-border-l4)')
  })
  ...
  it('matches the DSH plugin card surfaces in both color modes', async () => {
    const card = await readFile(new URL('../../src/components/CodexAuthCard.tsx', import.meta.url), 'utf8')
    const style = await readFile(new URL('../../src/styles/index.scss', import.meta.url), 'utf8')
    expect(style).toContain('var(--dsw-alias-bg-layer-3)')
    expect(style).toContain('var(--dsw-alias-bg-layer-2)')
    expect(style).toContain('border-radius: 12px')
    expect(style).toContain('padding: 14px 16px')
  })
