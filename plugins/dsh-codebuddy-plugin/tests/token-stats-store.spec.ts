 ...
 */
/**
 * 把 `resolveRange` 的端点转回 days 数（与服务端兼容 form 一致），便于断言
 * 「同一范围只请求一次」。
 *
 * `resolveRange` 现在必填传两个端点，这里也按必填消费；调用点不会缺端点
 * （store 拿到的总是 `resolveRange(key)` 的结果），断言里返回 0 仅是兜底，
 * 提示「fixture 写错了」而不是「天数真是 0」。
 */
function spanDays(req: { startTime: number, endTime: number }): number {
  return Math.round((req.endTime - req.startTime) / 86_400_000) + 1
}
...
function makeRpc(options: { fail?: boolean, delayMs?: number } = {}): { rpc: ConnectionRpc, calls: Array<{ days: number, allTime?: boolean }> } {
  ...
  const rpc: ConnectionRpc = {
    call: async <T>(_channel: string, _endpoint: string, payload?: unknown): Promise<RpcResult<T>> => {
      const req = payload as { startTime: number, endTime: number, allTime?: boolean }
      const allTime = req.allTime === true
      const days = allTime ? 0 : spanDays(req)
      calls.push(allTime ? { days, allTime: true } : { days })
      ...
      const tag = allTime ? 'all' : `d${days}`
      return { ok: true, value: { rangeDays: days, allTime, tag } as unknown as T }
    },
  }
  return { rpc, calls }
}
  ...
  it('reload 后拿到的是新数据，而不是沿用旧值', async () => {
    ...
    const rpc: ConnectionRpc = {
      call: async <T>(_c: string, _e: string, payload?: unknown): Promise<RpcResult<T>> => {
        round += 1
        return { ok: true, value: { rangeDays: spanDays(payload as { startTime: number, endTime: number }), tag: `round-${round}` } as unknown as T }
      },
    }
    const store = new TokenStatsStore(rpc)
    store.ensure('30d')
  ...
  it('刷新失败时保留旧数据并标记错误（供 UI 提示），而不是清空', async () => {
    ...
    const rpc: ConnectionRpc = {
      call: async <T>(_c: string, _e: string, payload?: unknown): Promise<RpcResult<T>> => {
        ...
        return { ok: true, value: { rangeDays: spanDays(payload as { startTime: number, endTime: number }), tag: 'ok' } as unknown as T }
      },
    }
    const store = new TokenStatsStore(rpc)
    store.ensure('30d')
