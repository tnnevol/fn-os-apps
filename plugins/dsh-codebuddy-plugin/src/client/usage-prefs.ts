...
/** Read the automatic daily sign-in preference; defaults to on. */
export function getAutoCheckinPref(): boolean {
  try {
    return window.localStorage.getItem(CODEBUDDY_AUTO_CHECKIN_KEY) !== '0'
  } catch {
    return true
  }
}

/** Persist the automatic daily sign-in preference and notify subscribers. */
export function setAutoCheckinPref(value: boolean): void {
  try {
    window.localStorage.setItem(CODEBUDDY_AUTO_CHECKIN_KEY, value ? '1' : '0')
  } catch {
    // See setUsagePref.
  }
  emitUsagePref()
