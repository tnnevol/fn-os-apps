  ...
  <div class="semi-preview" :class="previewClass">
    <div class="semi-preview__hero">
      ...
      <div class="semi-preview__theme" role="group" aria-label="预览主题">
        <button type="button" :class="{ active: previewTheme === 'light' }" @click="previewTheme = 'light'">
          亮色
        </button>
        <button type="button" :class="{ active: previewTheme === 'dark' }" @click="previewTheme = 'dark'">
          暗色
        </button>
      </div>
    </div>
    ...
    <div class="semi-preview__layout">
      <aside class="semi-preview__nav" aria-label="组件分类">
        ...
        <button v-for="item in categories" :key="item[0]" type="button" :class="{ active: category === item[0] }" @click="category = item[0]">
          <span class="semi-preview__nav-dot" />{{ item[1] }}
        </button>
        <span class="semi-preview__nav-title semi-preview__nav-title--group">输入类</span>
        <button type="button" :class="{ active: category === 'selection' }" @click="category = 'selection'">
          Cascader 级联选择
        </button>
        <button type="button" :class="{ active: category === 'selection' }" @click="category = 'selection'">
          TreeSelect 树选择器
        ...
        <button type="button" :class="{ active: category === 'tree' }" @click="category = 'tree'">
          Tree 树形控件
        </button>
        <span class="semi-preview__nav-title semi-preview__nav-title--group">反馈类</span>
        <button type="button" :class="{ active: category === 'feedback' }" @click="category = 'feedback'">
          Progress / Spin / Toast
        </button>
      </aside>
      ...
      <div class="semi-preview__content">
        <section v-if="category === 'buttons'" class="semi-preview__section">
          ...
          <div class="semi-doc-example-note">
            代码示例
          </div>
...
&lt;Button type="primary" theme="solid"&gt;主要按钮&lt;/Button&gt;</code></pre>
          ...
          <div class="semi-preview__demo-group">
            ...
            <div class="semi-preview__row">
              <button v-for="item in buttonTypes" :key="item[0]" type="button" class="semi-button is-solid" :class="`is-${item[0]}`">
                {{ item[1] }}
              </button>
            </div>
          </div>
          ...
          <div class="semi-preview__demo-group">
            ...
            <div class="semi-preview__row">
              <button v-for="item in buttonThemes" :key="item" type="button" class="semi-button is-primary" :class="`is-${item}`">
                {{ item }}
              </button>
            </div>
          </div>
          ...
          <div class="semi-preview__demo-group">
            ...
            <div class="semi-preview__row">
              <button v-for="item in buttonSizes" :key="item" type="button" class="semi-button is-secondary is-solid" :class="`is-size-${item}`">
                {{ item }}
              </button>
            </div>
          </div>
          ...
          <div class="semi-preview__demo-group">
            ...
            <div class="semi-preview__row">
              <button type="button" class="semi-button is-primary is-solid is-loading">
                <span class="semi-spinner" />加载中
              </button>
              <button type="button" class="semi-button is-secondary is-solid" disabled>
                禁用
              </button>
              <button type="button" class="semi-button is-danger is-outline" disabled>
                禁用描边
              </button>
              <button type="button" class="semi-button is-primary is-solid semi-button--wide">
                块级按钮
              </button>
            </div>
          </div>
          ...
          <div class="semi-preview__demo-group">
            ...
            <div class="semi-preview__row">
              <button type="button" class="semi-button is-primary is-solid">
                <span class="semi-icon">⚙</span>设置
              </button>
              <button type="button" class="semi-button is-secondary is-light">
                刷新 <span class="semi-icon">↻</span>
              </button>
              <button type="button" class="semi-icon-button is-primary is-solid" aria-label="设置">
                ⚙
              </button>
              <div class="semi-button-group" role="group" aria-label="操作按钮组">
                <button type="button">
                  保存
...
&lt;DshTreeSelect multiple treeCheckable checkRelation="related" /&gt;</code></pre>
          ...
          <div class="semi-preview__demo-group">
            ...
            <div class="semi-preview__row">
              ...
              <button type="button" class="semi-select" disabled>
                禁用 <span>⌄</span>
              </button>
              <button type="button" class="semi-select is-error">
                错误状态 <span>⌄</span>
              </button>
            </div>
          </div>
          ...
          <div class="semi-preview__demo-group">
            ...
            <div class="semi-preview__row">
              ...
              <button type="button" class="semi-select">
                多选 <span class="semi-tag">插件</span><span class="semi-tag">应用</span><span>⌄</span>
              </button>
              <button type="button" class="semi-select is-success">
                成功状态 <span>⌄</span>
              </button>
            </div>
          </div>
...
&lt;DshTree treeData={treeData} defaultExpandAll showLine /&gt;</code></pre>
          <div class="semi-tree-demo">
            <div class="semi-tree-row semi-tree-row--root">
              <span>⌄</span><span class="semi-checkbox is-indeterminate">−</span><span class="semi-folder">▰</span><strong>工作区</strong>
            </div>
            <div v-for="node in treeNodes" :key="node.value" class="semi-tree-row">
              <span class="semi-tree-indent">›</span><button type="button" class="semi-checkbox" :class="{ checked: selectedTree.includes(node.value) }" @click="toggleTree(node.value)">
                {{ selectedTree.includes(node.value) ? '✓' : '' }}
              </button><span class="semi-folder">{{ node.icon }}</span><span>{{ node.label }}</span>
            </div>
          </div>
          <div class="semi-preview__demo-group">
            <span class="semi-preview__label">共享图标</span><div class="semi-icon-grid">
              <span><b>▰</b>文件夹</span><span><b>▱</b>打开文件夹</span><span><b>▤</b>文件</span><span><b>⚙</b>设置</span><span><b>↻</b>刷新</span><span><b>↗</b>重启</span>
            </div>
          </div>
        </section>
        ...
        <section v-else-if="category === 'feedback'" class="semi-preview__section">
          ...
          <div class="semi-feedback-progress">
            <span style="width: 25%" />
          </div>
          <div class="semi-feedback-progress">
            <span class="is-success" style="width: 68%" />
          </div>
          <div class="semi-feedback-progress is-large">
            <span class="is-warning" style="width: 88%" />
          </div>
          <div class="semi-doc-example-note">
            Spin 加载器
          </div>
          <div class="semi-preview__row">
            <span class="semi-spinner" /><span class="semi-spinner semi-spinner--large" /><span class="semi-preview__label">加载中</span>
          </div>
          <div class="semi-doc-example-note">
            Toast 提示
          </div>
          <div class="semi-preview__row">
            <button type="button" class="semi-button is-secondary is-light" @click="toastVisible = true">
              显示 Toast
            </button><div v-if="toastVisible" class="semi-feedback-toast" role="alert">
              操作已完成 <button type="button" aria-label="关闭 Toast" @click="toastVisible = false">
                ×
              </button>
            </div>
          </div>
...
&lt;DshModal title="确认操作" visible={visible} /&gt;</code></pre>
          <button type="button" class="semi-button is-primary is-solid" @click="modalOpen = true">
            打开 Modal
          </button>
          <div v-if="modalOpen" class="semi-modal-mask" @click.self="modalOpen = false">
            <div class="semi-modal" role="dialog" aria-modal="true" aria-label="DSH Semi UI">
              <div class="semi-modal__heading">
                <strong>DSH Semi UI</strong><button type="button" aria-label="关闭" @click="modalOpen = false">
                  ×
                </button>
              </div><p>Modal 使用统一的 DSH 主题 Token。</p><div class="semi-modal__actions">
                <button type="button" class="semi-button is-secondary is-light" @click="modalOpen = false">
                  取消
                </button><button type="button" class="semi-button is-primary is-solid" @click="modalOpen = false">
                  确认
                </button>
              </div>
            </div>
