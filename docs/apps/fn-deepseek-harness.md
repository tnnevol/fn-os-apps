...
- Web profile 在首次执行 `dsh plugin --profile web add/update` 时由官方 CLI 自动初始化，插件通过 `dsh plugin --profile web add/update/remove` 管理，不再执行应用自定义插件脚本。选择 FPK 内置插件时，只将仓库中的本地插件制成 npm 归档并通过 DSH CLI 的 `file:` spec 安装，确保运行依赖可被解析；旧版 `link:` 安装会在升级时修复。清单中的三方插件不进入 FPK，仍由 DSH CLI 单独安装。安装回调在 `${TRIM_APPDEST}/app/bin/dsh` 创建公开 CLI wrapper，再由 `usr-local-linker` 注册到系统 bin。
...
- `dsh` wrapper 在安装回调中生成，只供用户调用 CLI，固定应用用户、运行环境和 `${TRIM_PKGHOME}` 路径。`cmd/main` 只启动网关；网关以应用包用户直接运行真实 DSH CLI 启动 Web，捕获本轮启动 Token 后持久化并用于页面、HTTP、SSE 和 WebSocket 代理。
