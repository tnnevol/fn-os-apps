...
- 内容输入框右下角使用彩色 fnOS 官方 logo 作为入口，按钮通过 DSH 的 `conversation.input.right` 插槽加入，不覆盖其他插件按钮。
- 点击后直接打开插件自有的 DSH 风格授权路径面板；面板通过 `/plugins/dsh-fnos/authorized-directories/entries` 列出授权根目录及其下一级文件/目录，支持进入目录和多选。
- 该入口不调用 fnOS SDK 文件/目录选择器，也不弹出“选择文件/选择目录”的二级菜单；只有授权范围内的 NAS 路径可被选择。
- 选中的文件和目录以 DSH 原生引用插入输入框，显示可读路径，支持去重和移除；取消面板不会改变原有输入内容。
