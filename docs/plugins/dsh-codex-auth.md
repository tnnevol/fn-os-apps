...
等待授权期间，按钮变成「取消」。点它、或者直接关掉授权窗口，都会放弃本次登录并退回未登录状态——这两种情况都不会删除已经登录的账号；只有「退出登录」才会清除凭据。点「取消」时若授权窗口还开着，插件会把它一并关掉（重复点「打开授权页面」开出的多个窗口也会全部关闭）；窗口已全部关掉则页面提示「授权窗口已关闭，登录未完成」。
