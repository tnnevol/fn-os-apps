...
登录成功后，面板会显示每周剩余用量和重置时间；接口明确返回五小时窗口时，会同时显示五小时剩余用量和重置时间。点击「刷新」可以立即更新；登录等待状态和账号用量也会自动刷新。对话输入区下方的状态栏只保留一个紧凑用量状态，优先显示五小时窗口，没有五小时窗口时显示每周窗口。
...
Codex Auth 面板新增「刷新模型目录」按钮。点击后插件读取当前 ChatGPT 账号**实际可用**的 Codex 模型与思考强度，并同步到 DSH 的 OpenAI Codex 模型配置；刷新成功后，全局模型选择器、会话内模型选择和「设置 → 模型」目录都会显示账号当前可用的模型。内部服务型条目（如自动审查、预留位）不会写入，思考级别只保留 DSH 支持的范围。

- 刷新需要已登录；未登录时按钮不显示。
- 刷新失败（网络异常、接口不可用、写入被拒绝）会保留上一次有效的模型列表，并在面板显示错误提示。
...
- 确认面板显示「已登录」；刷新按钮只在登录后出现。
- 刷新成功后需在模型选择器中重新查看——DSH 会在写入后重建模型目录，但已打开的页面可能需要刷新一次。
