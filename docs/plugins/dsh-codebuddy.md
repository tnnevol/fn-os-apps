...
- **防重入**：一轮要串行访问 N 个账号的远端接口，只要耗时超过间隔，下一轮就会叠加上来，同一账号被并发派发或重复领取。每个周期用 `RunGuard`（对照 workbuddy-switch 的 `RunFlagGuard`）做「检查并置位」，拿不到标志就返回 `skipped` 并直接退出。派发与领取各有独立标志，互不阻塞。旅行派发的首轮与领取周期刻意不同时启动——否则启动瞬间同一个 `arrived` 账号会被领取两次。
