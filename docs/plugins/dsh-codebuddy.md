...
「**添加账号入口**位于「账号管理」区块标题右侧的动作区（与自动签到/自动旅行/刷新同排），原先页面顶部那张独立操作卡已移除。区块头**常驻渲染**，不随「有账号」条件渲染——否则账号数为 0 时（恰恰最需要添加账号）入口会消失；列表为空时只把卡片网格换成空状态。动作区现含四个控件，窄屏一行放不下，因此设了 `flex-wrap: wrap` 并在 ≤720px 竖排时改为左对齐。
