...
- **当前账号隐藏切换入口**：已选中的账号不再渲染「选择账号」按钮（切换在途时仍渲染，保留「切换中…」与转圈，否则点了按钮它会直接消失）。此前该按钮无条件渲染，而它的 `disabled` 里含 `... && account.id !== activeId`，对当前账号恒为 false，于是非切换状态下**可点却无效果**（自己切自己）。面板卡片的同名菜单项早已按 `!row.active && !autoSwitch` 隐藏，设置区块与之对齐。两处取舍不同：菜单项在开启自动切换时对**所有**账号隐藏（账号由策略接管，手动指定会被下一次自动切换覆盖），设置区块只隐藏当前账号，其它账号的手动切换仍保留。
