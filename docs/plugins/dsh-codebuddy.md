...
**「模型用量排行」面板已移除**：维度可切换后它与分布面板能力完全重合（同数据源、同两维度），保留两个只会同屏出现镜像数据；分布面板独占一行，「按模型」视角保留在维度切换的第二档。**时间档位收敛为「今天 / 近 7 天 / 近 30 天」（默认今天）**——「总计 / 本月 / 近 90 天」移除；`custom` 档由日期范围选择器激活（终点视为今天，窗口 = 起点相对今天的天数，服务端按 `days` 计算无需改动）。
