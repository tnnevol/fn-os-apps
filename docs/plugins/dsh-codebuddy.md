...
- 悬停显示「已用额度 / 总量」和重置时间；点击展开浮层按计量窗口列出剩余比例；达到告警阈值变红。
- 偏好：**自动切换**（额度不足或被限流时切换账号）、**切换阈值**（剩余百分比滑块，0 关闭主动切换）、**显示额度余量**、**余量告警百分比**（默认 90%）。
...
- **账号管理**：顶部提供 OAuth 添加账号操作，账号区显示数量、刷新入口和响应式卡片网格；卡片包含头像、名称/环境、签到状态、剩余额度、资源包进度，以及单账号签到、「设为当前」（**无可用余额时禁用**）、删除。签到走 meter 平面 checkin-activity-status / daily-checkin，与 workbuddy-switch 同一协议。
