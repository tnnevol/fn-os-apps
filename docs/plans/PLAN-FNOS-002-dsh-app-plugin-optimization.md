...
- `FNOS-002-04`：使用 `connect` 与 `http-proxy-middleware` 重写 fnOS 统一网关代理；由常驻网关承载 FPK 状态并提供 DSH Web 恢复入口；由 fnOS 插件管理三方插件 API URL 反代规则，并让已打开的 DSH 页面立即取得最新配置。
...
总览插件的 `typecheck`、`test` 和 `build` 命令在包名确认并创建骨架后补入本节。
