...
5. 在模拟 profile 上验证清单插件目录、bundle 名称和已有用户 bundle 均被正确保留；npm 安装前将有效的 `@tnnevol/dsh-fnos` `link:` 本地依赖规范化为 `file:`，仅移除路径失效或旧 patch 不可用的 bundle（不删除插件包或用户数据），同时确认 FPK 不包含 `app/plugins` 本地产物。
