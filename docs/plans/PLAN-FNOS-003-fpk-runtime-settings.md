...
| PLAN-FNOS-003-C18 | 需求 002：自动切换开启时隐藏设置页与管理面板的所有手动切换入口 | 两个入口都按同一个 `autoSwitch` 状态隐藏；关闭自动切换后，符合余额条件的手动入口恢复；需求已验收 |
| PLAN-FNOS-003-C19 | 需求 003：添加账号登录在途时禁用整表单，并在重新打开、成功、超时或失败后解除 | 五个表单字段均受 `waiting` 控制；关闭时作废在途握手，避免取消后继续开窗或启动轮询；需求已验收 |
...
| P1 CodeBuddy 多账号与管理面板 | <Badge type="tip" text="代码已实现，需求 002/003 已验收" /> | 多账号、自动切换、签到、额度/有效期和 Token ECharts 面板本地检查通过；需求 002/003 已完成 DSH 客户端验收 |
