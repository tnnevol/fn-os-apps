...
| 任务 ID | 对应验收 | 实现内容 | 验收 |
| --- | --- | --- | --- |
| PLAN-FNOS-005-T10-01 | FNOS-005-10-AC-01/02 | 底部 SideSheet + CodeHighlight 展示逐条日志；点「完成任务」自动展开，动作区不额外放日志按钮 | 每行含时间/账号/任务/状态 |
| PLAN-FNOS-005-T10-02 | FNOS-005-10-AC-03/04/05 | 宿主每条处理完即 `appendGrowthRunLog` 落盘（串行队列、条数上限）；抽屉仅在展开且执行中轮询，begin 清空上一轮 | 执行中逐条追更，不丢日志不空转 |
...
| P1 任务执行日志抽屉 | <Badge type="tip" text="已完成" /> | 底部抽屉 + 等宽代码块，执行中追更、跑完回看 |
