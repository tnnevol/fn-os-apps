...
| PLAN-FNOS-005-T10-03 | FNOS-005-10-AC-06/07/09/10 | 抽屉占 `50vh`，上半屏用 DSH mask token + `backdrop-filter` 做半透明磨砂；日志区固定高度内部滚动，内容区 `border-box` 消除假滚动条；深色底与滚动条同层，状态按语义着色 | 背景可辨认，短日志无滚动条，滚动条不显溢出 |
