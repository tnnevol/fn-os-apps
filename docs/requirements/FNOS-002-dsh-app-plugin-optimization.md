...
- Codex 等待授权期间，「登录」按钮变为「取消」。用户点「取消」或关掉授权窗口都视为放弃本次登录：中止 device-code 轮询、清除本次一次性授权码、界面退回未登录态。该路径**不得**删除已保存的凭据（退出登录才清除），两者是不同操作。
- 点「取消」时若授权窗口仍然存在，插件把它一并关闭；重复点「打开授权页面」开出的多个窗口全部关闭。因此授权窗口**不得**切断 `opener`（`noopener` 或 `opener = null` 都会让跨域后的窗口失去 script-closable 资格，`close()` 静默失效）。判定「用户关了窗口」以**全部**授权窗口都关闭为准。
...
- Codex 等待授权时点「取消」或关闭授权窗口，界面及时退回未登录态且已保存凭据不受影响；点「取消」会关闭仍存在的授权窗口；取消不会把中止当作错误提示。
...
| 2026-09-14 | 增加授权取消 | 等待授权时提供「取消」，并在授权窗口被关闭时自动放弃本次登录；取消只回收本次授权，不删除已保存凭据；登录按钮文案由「去登录」改为「登录」 |
