...
| FNOS-005-10 | P1 | 任务执行日志抽屉 | 点「完成任务」自动展开底部抽屉（占下半屏、上半屏磨砂），用等宽代码块逐条展示日志并在内部滚动；另有「查看日志」常驻入口 | <Badge type="tip" text="已完成" /> |
...
- **领养（`first_buddy`）必须先于旅行**：它产出的 Buddy 是旅行派发的前提，没有猫猫时派发会被服务端拒（`no active buddy`）。上游 API 返回顺序不含依赖保证，因此执行顺序由本插件显式指定。
...
- `FNOS-005-06-AC-05`：执行顺序为「领养（`first_buddy`）→ 其余成长任务 → 签到 → 旅行」；领养不依赖上游返回顺序，由插件显式排序保证。
...
- `FNOS-005-10-AC-07`：日志区固定高度并在内部滚动；抽屉的标题与状态行常驻，长日志不把标题顶出视口。
...
| 任务执行日志抽屉 | <Badge type="tip" text="已完成" /> | 底部 SideSheet（50vh、上半屏磨砂）+ 等宽代码块；日志区固定高度内部滚动；含「查看日志」按钮 | 已完成；实现、测试和构建通过 |
