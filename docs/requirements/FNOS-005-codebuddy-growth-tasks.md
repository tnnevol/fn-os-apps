...
- Host 侧不展示或新增「自动签到」「自动旅行」配置项；本需求只移植成长任务能力，不复制截图中的两个运营周期开关。
...
| Host 配置边界 | `plugins/dsh-codebuddy-plugin/src/host`、`src/client/ui` | 不新增或透出自动签到、自动旅行两个配置项，避免与既有运营周期能力重复 |
...
| FNOS-005-05 | P1 | Host 侧配置展示边界 | 不展示或新增截图中的「自动签到」「自动旅行」配置项，成长任务功能不复制既有运营周期设置 | <Badge type="tip" text="已完成" /> |
...
- Host 侧不注册新的「自动签到」和「自动旅行」配置项，不在成长任务页面渲染它们；既有运营周期的持久化偏好保持原有行为，不因本需求被修改。
...
- `FNOS-005-05-AC-01`：Host 侧成长任务相关页面不展示「自动签到」「自动旅行」两个开关，也不新增对应配置入口。
- `FNOS-005-05-AC-02`：成长任务移植不新增、不修改这两个既有运营周期偏好；已有其它页面的运营能力保持原边界。
...
| Host 侧配置展示边界 | <Badge type="tip" text="已完成" /> | 不展示、不新增、不修改自动签到/自动旅行配置 | 已完成；实现、测试和构建通过 |
...
| 2026-09-14 | 补充 Host 侧配置边界 | 明确成长任务移植不展示、不新增、不持久化「自动签到」「自动旅行」配置项 |
