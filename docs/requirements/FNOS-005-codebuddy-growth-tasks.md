...
| FNOS-005-10 | P1 | 任务执行日志抽屉 | 点「完成任务」自动展开底部抽屉，用等宽代码块逐条展示执行日志；执行中实时追更 | <Badge type="tip" text="已完成" /> |
...
- `FNOS-005-10-AC-01`：点击「完成任务」自动展开底部日志抽屉；动作区不额外放日志按钮（抽屉只由该动作唤起），关闭后日志仍在宿主保留，下一次执行会重置。
...
| 任务执行日志抽屉 | <Badge type="tip" text="已完成" /> | 底部 SideSheet + 等宽代码块展示逐条日志；执行中实时追更，无独立按钮 | 已完成；实现、测试和构建通过 |
