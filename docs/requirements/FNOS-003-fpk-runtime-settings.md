...
- 需求 002 验收：自动切换开启时，设置页与管理面板对所有账号隐藏手动切换入口；关闭自动切换后，符合余额条件的手动切换入口恢复。
- 需求 003 验收：添加账号点击「打开登录」后，备注名、客户端、环境、企业服务地址和企业开关全部禁用；重新打开弹框、登录成功、授权轮询超时或失败后解除禁用，关闭弹框不会继续执行已取消的握手。
