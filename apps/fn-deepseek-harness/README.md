...
FPK 只处理 [`app/published-dsh-plugins.json`](app/published-dsh-plugins.json) 中声明的已发布插件。安装和升级阶段使用安装向导选择的 npm 源：只有 `profile web` 中缺少清单插件时，才会通过 npm 按清单中的精确版本或 dist-tag 安装；已有本地插件会被保留，不会被线上包覆盖；随后统一补齐 `dsh.profile.bundles`。应用启动只校验已安装版本，不会每次启动联网。当前清单使用 `@tnnevol/dsh-codex-auth` 和 `@tnnevol/dsh-fnos` 的 `rc` dist-tag，因此后续发布新的 rc 版本不需要修改 FPK 清单。未发布插件不会在 FPK 构建阶段编译、打包或复制到 Web profile，发布后需要先加入该清单才会随应用安装。
...
```text
fn-deepseek-harness-v<app-version>-dsh-0.1.1-rc.2.fpk
```

其中 `-dsh-` 后的版本就是 FPK 内置并在 NAS 上安装的 DSH 版本。构建产物中的 `pty.node` 不提交到源码仓库，由 workflow 在打包前按 `.github/config/dsh-native-0.1.1-rc.2.env` 生成。
