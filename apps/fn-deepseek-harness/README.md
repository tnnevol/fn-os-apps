...
GitHub Actions 在构建 `fn-deepseek-harness` 时会执行 [`.github/scripts/prepare-dsh-native.sh`](../../.github/scripts/prepare-dsh-native.sh)，构建参数维护在 [`.github/config/dsh-native-0.1.2-alpha.4.env`](../../.github/config/dsh-native-0.1.2-alpha.4.env)：
...
```text
fn-deepseek-harness-v<app-version>-dsh-0.1.2-alpha.4.fpk
```

其中 `-dsh-` 后的版本就是 FPK 内置并在 NAS 上安装的 DSH 版本。构建产物中的 `pty.node` 不提交到源码仓库，由 workflow 在打包前按 `.github/config/dsh-native-0.1.2-alpha.4.env` 生成。
