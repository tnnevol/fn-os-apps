...
export default defineEventHandler(async (event) => {
  ...
  if (!existsSync(openlistDataDir)) {
    throw createError({
      statusCode: 400,
      message: "openlist 数据目录不存在，请先运行 OpenList",
    });
  }
  ...
  await execAsync(`cd "${openlistDataDir}/.." && zip -r "${zipPath}" data`);
  ...
  setHeader(
    event,
    "Content-Disposition",
    `attachment; filename="openlist-backup-${timestamp}.zip"`,
  );
